...
class ClientList extends Component {
  renderClients(){
    ...
    return this.props.clients.map(
      client => <LinkCard key={client.id} client={client} customStyle={{width: '47%', height: 60, margin: 5}} onPress={()=> this.props.updateEventClient(client, this.props.currentEventId)}> {client.name}</LinkCard>
    )
  }
  render() {
